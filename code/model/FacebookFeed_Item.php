		...
		foreach($html->find('text') as $element) {
			...
			if(! in_array($element->parent()->tag, array('a', 'img'))) {
				if(phpversion() < 5.4) {
					...
					$element->innertext = preg_replace_callback("#(www(\S*?\.\S*?))(\s|\;|\)|\]|\[|\{|\}|,|\"|'|:|\<|$|\.\s)#ie", "'http://$1$4'", $element->innertext);
					$element->innertext = preg_replace_callback("#((http|https|ftp)://(\S*?\.\S*?))(\s|\;|\)|\]|\[|\{|\}|,|\"|'|:|\<|$|\.\s)#ie", "'<a href=\"$1\" target=\"_blank\">click here</a>$4'", $element->innertext);
				}
				else {
